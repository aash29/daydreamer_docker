...
class Config(dict):
    """Валидирует входящие конфигурационные данные."""

    SEP = '.'
    # Отрицание элементов в скобках
    IS_PATTERN = re.compile(r'.*[^A-Za-z0-9_.-].*')

    def __init__(self, *args, **kwargs):
        # Преобразует именованные аргументы в словарь.
        ...
        # Необходимо присвоить значения словарю базового класса, чтобы
        # преобразование в словарь не приводило к потере содержимого.
    ...
    def __str__(self):
        # Выводит в консоль форматированные данные.
        lines = ['\nConfig:']
        keys, vals, typs = [], [], []
    ...
    def update(self, *args, **kwargs):
        # Заменяет старое значение ключа на новое. Возвращает обновленный словарь.
    ...
    def _flatten(self, mapping):
        """Проверяет наличие символов отсутствующих в паттерне и возвращает словарь
    ...
    def _nest(self, mapping):
        # Создает вложенные словари, если ключ содержит точку в названии.
        result = {}
        for key, value in mapping.items():
    ...
    def _ensure_keys(self, mapping):
        """Проверяет на недопустимые символы в ключах. Возвращает словарь с данными конфигурации."""
        for key in mapping:
            assert not self.IS_PATTERN.match(key), key
        return mapping

    def _ensure_values(self, mapping):
        """Проверяет значения словаря на корректность входящих данных, на отсутствие данных.
        Преобразует значения в кортежи. Возвращает словарь."""
        # Преобразует строку в формате JSON в объект Python.
    ...
    def _format_value(self, value):
        # Преобразует значение (список, кортеж) в строковый тип данных.
        if isinstance(value, (list, tuple)):
            return '[' + ', '.join(self._format_value(x) for x in value) + ']'
        return str(value)

    def _format_type(self, value):
        # Возвращает имя типа данных. Если значение является списком или кортежем, преобразует в строковый тип данных
        # и добавляет на конце 's' (множественное число).
        if isinstance(value, (list, tuple)):
