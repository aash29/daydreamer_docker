...
class Flags:
    """Преобразует входные данные из командной строки и именованные аргументы в необходимые типы данных"""

    def __init__(self, *args, **kwargs):
        # Входные данные инициализируются классом Config.
        self._config = config.Config(*args, **kwargs)

    def parse(self, argv=None, help_exists=True):
        """Объединяет конфигурационные данные и аргументы командной строки в единый словарь."""
    ...
    def parse_known(self, argv=None, help_exists=False):
        # Возвращает словарь из именованных аргументов и список из аргументов командной строки.
        if argv is None:
            argv = sys.argv[1:]
        if '--help' in argv:
            print('\nHelp:')
        ...
        self._submit_entry(key, vals, parsed, remaining)
        # Добавляет в базовый словарь конфигурации словарь с наименованием робота
        parsed = self._config.update(parsed)
        return parsed, remaining

    def _submit_entry(self, key, vals, parsed, remaining):
        # Создает словарь с ключом configs и название робота
    ...
    def _parse_flag_value(self, default, value, key):
        # Возвращает название робота в виде кортежа
